...
export default function OrderCard({ order }) {
  ...
  const productNames = items.map(item => productName(item.product)).filter(Boolean)
  const stageHints = {
    pending: t('orders.waitingReview'),
    approved: t('orders.approvedScheduled'),
    ready: t('orders.readyPickup'),
    ...
    <Link to={`/member/orders/${order.id}`} className="block">
      <article className="group rounded-lg border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-amber-300 hover:shadow-lg">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-bold uppercase tracking-wide text-slate-500">{t('orders.order')} #{order.id}</p>
            <h3 className="mt-1 text-lg font-black text-slate-950">{stageHints[order.status] || t('orders.processing')}</h3>
          </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 rounded-lg bg-slate-50 p-3 text-sm">
          ...
          <div className="flex items-center gap-2 text-slate-600">
            ...
            {items.length || 0} {t('common.items')}
          </div>
        </div>

        <p className="mt-4 line-clamp-1 text-sm text-slate-500">
          {productNames.length > 0 ? productNames.join(', ') : t('orders.openDetails')}
        </p>
        ...
        <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-4">
          ...
          <span className="inline-flex items-center gap-1 text-sm font-bold text-amber-700">
            {t('common.details')} <ChevronRight className="transition group-hover:translate-x-0.5" size={16} />
          </span>
        </div>
      </article>
    </Link>
