...
export default function ProductDetail() {
  ...
  const fetchProduct = async () => {
    ...
    try {
      ...
      setToast({ type: 'error', message: 'Product could not be loaded' })
    } finally {
      setLoading(false)
    }
  }
  ...
  const orderSingleProduct = async () => {
    if (!user?.is_verified) {
      setToast({ type: 'error', message: 'Please complete verification before buying.' })
      return
    }
    ...
    try {
      ...
      setToast({ type: 'error', message: err.response?.data?.error || err.response?.data?.message || 'Failed to place order' })
    }
  }
    ...
    return (
      <AppLayout>
        <div className="rounded-lg border border-slate-200 bg-white p-8 text-center shadow-sm">
          ...
          <Button as={Link} to="/member/products" className="mt-4">Back to products</Button>
        </div>
      </AppLayout>
    )
  }
        ...
        <div className="grid gap-0 lg:grid-cols-[1.05fr_0.95fr]">
          ...
          <div className="flex flex-col p-5 sm:p-8">
            <div className="flex items-center justify-between gap-3">
              <p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{product.category || 'Product'}</p>
              <StockBadge quantity={product.quantity} />
            </div>

            <h1 className="mt-4 text-2xl font-black leading-tight text-slate-950 sm:text-4xl">{product.name}</h1>
            <p className="mt-4 text-sm leading-6 text-slate-600 sm:text-base">{product.description || 'Quality Shemachoch product.'}</p>

            <div className="mt-6 rounded-lg border border-slate-200 bg-slate-50 p-4">
              <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Member price</p>
              ...
              <p className="mt-2 text-sm font-semibold text-slate-600">{product.quantity} available in stock</p>
            </div>

            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              <Button onClick={orderSingleProduct} disabled={!user?.is_verified || Number(product.quantity) <= 0} className="py-3">
                <ShoppingCart size={18} />
                {Number(product.quantity) <= 0 ? 'Unavailable' : user?.is_verified ? 'Order one item' : 'Verify to buy'}
              </Button>
            ...
            {!user?.is_verified && (
              <p className="mt-3 rounded-lg bg-amber-50 p-3 text-xs font-bold leading-5 text-amber-800">
                Verification is required before placing orders.
              </p>
            )}
          </div>
        </div>
