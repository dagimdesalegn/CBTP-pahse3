...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setToast({ type: 'error', message: error.response?.data?.error || t('auth.loginFailed') })
    } finally {
      setLoading(false)
    }
  }
        ...
        <section className="hidden bg-slate-950 p-10 text-white lg:flex lg:flex-col lg:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-slate-400">{t('auth.portalSubtitle')}</p>
            </div>
          </div>
          <div className="max-w-xl">
            <p className="text-sm font-bold uppercase tracking-[0.22em] text-amber-300">{t('auth.memberCommerce')}</p>
            <h1 className="mt-4 text-5xl font-black leading-tight">{t('auth.heroTitle')}</h1>
            <p className="mt-5 text-lg leading-8 text-slate-300">{t('auth.heroCopy')}</p>
          </div>
          <div className="grid grid-cols-3 gap-3 text-sm">
            {[t('auth.liveInventory'), t('auth.secureCheckout'), t('auth.adminReports')].map(item => (
        ...
        <main className="flex items-center justify-center p-4">
          <div className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-6 shadow-xl sm:p-8">
            ...
            <div className="mb-8 text-center">
              ...
              <h1 className="mt-4 text-3xl font-black text-slate-950">{t('auth.signIn')}</h1>
              <p className="mt-2 text-sm text-slate-600">{t('auth.accessWorkspace')}</p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-5">
              <label>
                <span className="ui-label">{t('auth.email')}</span>
              ...
              <label>
                <span className="ui-label">{t('auth.password')}</span>
                <span className="relative block">
            ...
            <p className="mt-6 text-center text-sm text-slate-600">
              {t('auth.noAccount')} <Link to="/register" className="font-black text-amber-700">{t('auth.createOne')}</Link>
            </p>
          </div>
        </main>
      </div>
